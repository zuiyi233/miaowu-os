...
describe("设置 Hooks 测试", () => {
  beforeEach(() => {
    useSettingsStore.getState().resetSettings();
  });

  it("useActiveApiConfig 应该返回当前激活的 API 配置", () => {
    // 由于 useActiveApiConfig 是一个 hook，我们需要在 React 环境中测试
    // 这里我们直接测试 store 的逻辑
    const settings = useSettingsStore.getState();
  ...
  it("useModelConfig 应该返回指定任务的模型配置", () => {
    const settings = useSettingsStore.getState();
    const outlineConfig = settings.modelSettings.outline;
    
    expect(outlineConfig).toMatchObject({
      model: 'gemini-1.5-flash',
      temperature: 0.7,
      maxTokens: 4096,
    });

    const embeddingConfig = settings.modelSettings.embedding;
    expect(embeddingConfig).toMatchObject({
