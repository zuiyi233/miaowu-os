...
### 2026-05-10 增量同步快照（最新）

- 同步前本地基准：`5127f08e`（上次同步终点）
- 上游目标提交：`94da8f67`
- 同步 commit 数：1 个（`5127f08e..94da8f67`）
- 同步策略：子树补丁应用 + 低风险增量吸收 + 文档收口
- 差异结论：`deer-flow-main/` 子树已吸收 94da8f67 触达文件；本轮聚焦 dev 启动脚本与 UV_EXTRAS 保留机制
...
- 上游目标提交：`5127f08e`
- 同步 commit 数：17 个（`2b1fcb3e..5127f08e`）
- 同步策略：子树补丁应用 + 人工冲突裁决
- 差异结论：冲突文件收口后，二开小说流程与 8551 契约均保持
- 详细记录：`docs/upstream-sync-guide.md` 第 10 节
