...
def _set_csrf_cookie(response: Response, request: Request) -> None:
    """Set a JS-readable CSRF cookie after a GET callback creates a session."""
    response.set_cookie(
...
async def get_me(request: Request):
    ...
    user = await get_current_user_from_request(request)
    snapshot = await get_local_provider().get_newapi_snapshot(str(user.id))
    return UserResponse(
        id=str(user.id),
        email=user.email,
        system_role=user.system_role,
        needs_setup=user.needs_setup,
        newapi_account=snapshot,
    )


@router.get("/login/newapi")
async def login_newapi(next: str | None = None):
    """Start NewAPI OIDC login."""
    try:
        authorize_url = await build_newapi_authorize_url(next or "/workspace")
    except NewAPIOAuthError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.message) from exc
    return RedirectResponse(authorize_url, status_code=status.HTTP_302_FOUND)


@router.get("/callback/newapi")
async def callback_newapi(
    request: Request,
    code: str | None = None,
    state: str | None = None,
    error: str | None = None,
    ...
    """Complete NewAPI OIDC login and issue a Miaowu session cookie."""
    if error:
        detail = f"{error}: {error_description}" if error_description else error
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=detail)
    if not code or not state:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="NewAPI callback requires code and state")

    try:
        login_result = await exchange_newapi_code_for_user(code, state, get_local_provider())
    except NewAPIOAuthError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.message) from exc
