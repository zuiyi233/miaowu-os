...
class LocalAuthProvider(AuthProvider):
    ...
    async def create_oauth_user(self, email: str, provider: str, oauth_id: str, system_role: str = "user") -> User:
        """Create a local shadow user for a trusted OAuth/OIDC identity."""
        user = User(
            email=email,
            password_hash=None,
            system_role=system_role,
    ...
    async def get_newapi_snapshot(self, user_id: str) -> NewAPIAccountSnapshot | None:
        """Get the last synced NewAPI account snapshot for a local user."""
        return await self._repo.get_newapi_snapshot(user_id)

    async def upsert_newapi_snapshot(self, **kwargs) -> NewAPIAccountSnapshot:
        """Create or update the last synced NewAPI account snapshot."""
        return await self._repo.upsert_newapi_snapshot(**kwargs)
