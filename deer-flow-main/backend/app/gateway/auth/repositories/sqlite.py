...
class SQLiteUserRepository(UserRepository):
    ...
    @staticmethod
    def _snapshot_row_to_model(row: NewAPIAccountSnapshotRow) -> NewAPIAccountSnapshot:
        return NewAPIAccountSnapshot(
            user_id=row.user_id,
            newapi_sub=row.newapi_sub,
            email=row.email,
            ...
            balance=row.balance,
            last_synced_at=row.last_synced_at if row.last_synced_at.tzinfo else row.last_synced_at.replace(tzinfo=UTC),
        )
    ...
    async def get_newapi_snapshot(self, user_id: str) -> NewAPIAccountSnapshot | None:
        async with self._sf() as session:
            row = await session.get(NewAPIAccountSnapshotRow, user_id)
            return self._snapshot_row_to_model(row) if row is not None else None

    async def upsert_newapi_snapshot(
        self,
        *,
        user_id: str,
        newapi_sub: str,
        ...
        balance: int | None,
    ) -> NewAPIAccountSnapshot:
        now = datetime.now(UTC)
        async with self._sf() as session:
            row = await session.get(NewAPIAccountSnapshotRow, user_id)
            if row is None:
                row = NewAPIAccountSnapshotRow(user_id=user_id)
                session.add(row)
            row.newapi_sub = newapi_sub
            row.email = email
            ...
            row.remain_quota = remain_quota
            row.balance = balance
            row.last_synced_at = now
            await session.commit()
            await session.refresh(row)
            return self._snapshot_row_to_model(row)
