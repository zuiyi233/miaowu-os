...
def test_queue_add_stores_user_id():
    q = MemoryUpdateQueue()
    with patch("deerflow.agents.memory.queue.get_memory_config", return_value=MemoryConfig(enabled=True)):
...
def test_queue_process_passes_user_id_to_updater():
    ...
    with patch("deerflow.agents.memory.updater.MemoryUpdater", return_value=mock_updater):
        asyncio.run(q._process_queue())

    mock_updater.aupdate_memory.assert_awaited_once()
    call_kwargs = mock_updater.aupdate_memory.call_args.kwargs
