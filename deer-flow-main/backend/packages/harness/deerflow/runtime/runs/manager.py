...
class RunManager:
    ...
    @staticmethod
    def _sort_newest_first(records: list[RunRecord]) -> list[RunRecord]:
        """Sort newest-first and keep later insertions first when timestamps tie."""
        indexed_records = list(enumerate(records))
        indexed_records.sort(key=lambda item: (item[1].created_at, item[0]), reverse=True)
    ...
    async def list_by_thread(self, thread_id: str, *, user_id: str | None = None, limit: int = 100) -> list[RunRecord]:
        ...
        if self._store is None:
            return self._sort_newest_first(memory_records)[:limit]
        ...
        for row in rows:
            ...
            if run_id and run_id not in records_by_id:
                try:
                    ...
                    logger.warning("Failed to map store row for run %s", run_id, exc_info=True)
        return self._sort_newest_first(list(records_by_id.values()))[:limit]
