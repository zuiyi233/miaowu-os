...
- Extends `AgentState` with: `sandbox`, `thread_data`, `title`, `artifacts`, `todos`, `uploaded_files`, `viewed_images`, `execution_gate`
...
6. **ExecutionGateMiddleware** - Thread-level high-risk write gate for novel actions: question-priority answer-only turns, explicit authorization (`确认执行` / `进入执行模式`), revocation (`退出执行模式` / `取消授权`), and blocked-action replay
