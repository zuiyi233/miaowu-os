  ...
  while (true) {
    ...
    for (const line of buffer.split('\n')) {
      ...
      try {
        ...
        if (data.type === 'progress') callbacks.onProgress(data.progress || 0, data.word_count);
        else if (data.type === 'chunk') callbacks.onChunk(data.content || '');
        else if (data.type === 'result') callbacks.onResult(data.data || {});
        else if (data.type === 'error') callbacks.onError(data.message || 'Error');
      } catch {}
    }
  }
}
...
export function ChapterRegenerationModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await streamRegenerate(`/api/chapters/${chapterId}/regenerate-stream`, {
        modification_source: modSource,
        ...
        preserve_elements: {
          ...
          preserve_character_traits: preserveCharacterTraits,
        },
        target_word_count: targetWordCount,
        focus_areas: focusAreas,
      }, {
        onProgress: (prog, wc) => { setProgress(prog); if (wc) setWordCount(wc); },
        onChunk: (chunk) => { accumulated += chunk; setWordCount(accumulated.length); },
        onResult: (data) => {
      ...
      setStatus('error'); setErrorMsg(err instanceof Error ? err.message : '未知错误');
      toast.error('操作失败');
  ...
  return (
    <>
      ...
      <SSEProgressModal open={status === 'generating'} onOpenChange={() => {}} progress={progress} message={`正在重新生成中... (已生成 ${wordCount} 字)`} title="重新生成章节" showPercentage onCancel={() => {}} cancelButtonText="中断" />
    </>
  );
}
