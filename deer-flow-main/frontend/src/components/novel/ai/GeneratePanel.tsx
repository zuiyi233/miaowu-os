...
import { AlertCircle, BookOpen, Globe, Loader2, PencilLine, Sparkles, Square, Users } from 'lucide-react';
...
const GENERATION_TYPE_KEYS: { value: GenerationType; labelKey: 'worldview' | 'outline' | 'chapters' | 'characters'; icon: React.ReactNode; descKey: string }[] = [
  { value: 'worldview', labelKey: 'worldview', icon: <Globe className="h-4 w-4" />, descKey: 'worldviewDesc' },
  { value: 'outline', labelKey: 'outline', icon: <PencilLine className="h-4 w-4" />, descKey: 'outlineDesc' },
  { value: 'chapter', labelKey: 'chapters', icon: <BookOpen className="h-4 w-4" />, descKey: 'chapterDesc' },
  { value: 'characters', labelKey: 'characters', icon: <Users className="h-4 w-4" />, descKey: 'charactersDesc' },
];

let chaptersCache: { novelId: string; data: ChapterOption[] } | null = null;
...
export function GeneratePanel({ novelId }: { novelId: string }) {
  ...
  const [result, setResult] = useState<GenerationResult>({ content: '', status: 'idle' });
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled && chs.length > 0) {
          const opts = chs.map((c: ChapterOption) => ({ id: c.id, title: c.title, order: c.order }));
          setChapters(opts);
          chaptersCache = { novelId, data: opts };
    ...
    try {
      ...
      switch (generationType) {
        ...
        case 'chapter': {
          ...
          if (!targetChapterId) {
            setResult({ content: '', status: 'error', error: t.novel.noChapters });
            return;
          }
      ...
      if (error instanceof DOMException && error.name === 'AbortError') {
        flushChunks();
        setResult((prev) => (prev.content.length > 0 ? { ...prev, status: 'success' } : { content: '', status: 'idle' }));
        return;
      }
      const message = error instanceof Error ? error.message : t.novel.generationFailed;
      setResult({ content: '', status: 'error', error: message });
    } finally {
      abortRef.current = null;
    }
  }, [generationType, novelId, prompt, chapterId, chapters, t, scheduleFlush, flushChunks]);
    ...
    <div className="flex h-full flex-col">
      <ScrollArea className="flex-1">
        <div className="space-y-4 p-4">
          <div className="space-y-2">
            <Label className="text-xs font-medium text-muted-foreground">{t.novel.generationType}</Label>
          ...
          {generationType === 'chapter' && chapters.length > 0 && (
            <div className="space-y-2">
              <Label className="text-xs font-medium text-muted-foreground">{t.novel.targetChapter}</Label>
                ...
                <SelectContent>
                  {chapters.map((ch) => (
                    <SelectItem key={ch.id} value={ch.id}>
                      {ch.title || `${t.novel.chapterFallback} ${ch.order ?? ''}`}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          {generationType === 'chapter' && chapters.length === 0 && (
            <Alert variant="destructive">
              ...
              <AlertDescription className="text-xs">{t.novel.noChapters}</AlertDescription>
            </Alert>
          )}

          <div className="space-y-2">
            <Label className="text-xs font-medium text-muted-foreground">{t.novel.creativeRequirements}</Label>
            <Textarea
              value={prompt}
          ...
          {result.status === 'success' && result.content.length > 0 && (
            <Alert>
              ...
              <AlertDescription className="text-xs">{t.novel.generationComplete}</AlertDescription>
            </Alert>
          )}
              ...
              {result.status === 'success' && (
                <div className="mt-3 flex gap-2 flex-wrap">
                  {generationType === 'chapter' && (
                    <Button size="sm" variant="outline" onClick={handleApplyToChapter}>
                      {t.novel.applyToChapter}
                    </Button>
                  )}
                  {(generationType === 'outline' || generationType === 'worldview') && (
                    <Button size="sm" variant="outline" onClick={handleApplyToOutline}>
      ...
      <div className="border-t p-3">
        <div className="flex gap-2">
          {isGenerating ? (
            <Button size="sm" variant="destructive" onClick={handleAbort} className="w-full">
