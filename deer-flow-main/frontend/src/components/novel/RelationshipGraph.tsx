...
const EDGE_CATEGORY_COLORS: Record<EdgeCategory, { color: string; order: number }> = {
...
export function RelationshipGraph({ projectId }: RelationshipGraphProps) {
  ...
  const edgeCategoryMeta = useMemo(() => {
    const result: Record<EdgeCategory, { label: string; color: string; order: number }> = {} as any;
    for (const cat of Object.keys(EDGE_CATEGORY_COLORS) as EdgeCategory[]) {
      result[cat] = { label: edgeCategoryLabels[cat], ...EDGE_CATEGORY_COLORS[cat] };
    }
    return result;
  }, [edgeCategoryLabels]);
  ...
  const buildFlowElements = useCallback((
    ...
    if (hasMainCareers) {
      flowNodes.push({ id: '__cg_main__', type: 'careerGroupNode', position: { x: 0, y: 0 }, data: { label: t.novel.careerGroup, careerType: 'main' as const } });
    }
    if (hasSubCareers) {
      flowNodes.push({ id: '__cg_sub__', type: 'careerGroupNode', position: { x: 0, y: 0 }, data: { label: t.novel.subCareerGroup, careerType: 'sub' as const } });
    }
    ...
    (careerData.main_careers || []).forEach((c) => {
      ...
      if (hasMainCareers) {
        flowEdges.push(buildCategoryEdge(`__cg_main__-career-main-${c.id}`, '__cg_main__', `career-main-${c.id}`, `${t.novel.careerCategory}·${c.name}`));
      }
    });
    ...
    (careerData.sub_careers || []).forEach((c) => {
      ...
      if (hasSubCareers) {
        flowEdges.push(buildCategoryEdge(`__cg_sub__-career-sub-${c.id}`, '__cg_sub__', `career-sub-${c.id}`, `${t.novel.careerCategory}·${c.name}`));
      }
    });
  ...
  const edgeCategories = useMemo(() =>
    Object.entries(edgeVisibilityMap)
      .map(([cat, visible]) => ({ cat, visible, ...(edgeCategoryMeta[cat as EdgeCategory] || edgeCategoryMeta.default) }))
  ...
  const loadCharacterDetail = useCallback(async (nodeId: string) => {
    ...
    } catch { console.error(t.novel.loadCharacterDetailsFailed); }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h2 className="text-lg font-semibold flex items-center gap-2"><Network className="w-5 h-5" /> {t.novel.characterRelationshipGraph}</h2>
        <div className="flex items-center gap-2">
          <Badge variant="outline">{graphData?.nodes.length || 0} {t.novel.nodes}</Badge>
          <Badge variant="outline">{graphData?.links.length || 0} {t.novel.relationshipsLabel}</Badge>
          {(careers.main_careers?.length || 0) > 0 && <Badge variant="outline" className="text-yellow-600">{careers.main_careers?.length || 0} {t.novel.mainCareer}</Badge>}
          {(careers.sub_careers?.length || 0) > 0 && <Badge variant="outline" className="text-cyan-600">{careers.sub_careers?.length || 0} {t.novel.subCareer}</Badge>}
          <Button size="sm" variant="outline" onClick={saveCurrentLayout} title={t.novel.layoutSaved}><Save className="w-3.5 h-3.5 mr-1" />{t.common.save}</Button>
          {hasSavedLayout && <Button size="sm" variant="ghost" onClick={resetLayout}><RotateCcw className="w-3.5 h-3.5 mr-1" />{t.novel.reset}</Button>}
          <Button size="sm" variant="outline" onClick={loadAll}>{t.novel.refresh}</Button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <Select value={filterType} onValueChange={(v) => setFilterType(v as typeof filterType)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="organization">{t.novel.organizationsOnly}</SelectItem>
          </SelectContent>
        </Select>
        <Input placeholder={t.novel.searchName} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="max-w-[200px]" />

        {edgeCategories.length > 1 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-xs text-muted-foreground">{t.novel.connections}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <Card className={cn("lg:col-span-3 min-h-[520px]", loading && "animate-pulse")}>
          <CardContent className="pt-4 p-0 h-full relative overflow-hidden" style={{ minHeight: 520 }}>
            {loading ? (
              <div className="flex items-center justify-center h-[520px] text-muted-foreground">{t.novel.loading}</div>
              ...
              <div className="flex items-center justify-center h-[520px] text-muted-foreground text-sm">{t.novel.noRelationshipData}</div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            {selectedDetail ? (
              <ScrollArea className="max-h-[480px]">
                <DetailPanel detail={selectedDetail} connectedEdges={connectedEdges} graphData={graphData} careers={careers} roleLabels={roleLabels} edgeCategoryMeta={edgeCategoryMeta} t={t} />
              </ScrollArea>
            ) : selectedNode ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant={selectedNode.is_organization ? 'default' : 'secondary'}>{selectedNode.is_organization ? t.novel.organizations : roleLabels[selectedNode.role_type || ''] || t.novel.characters}</Badge>
                </div>
                <Separator />
                <p className="text-sm font-medium mb-2">{t.novel.relatedRelationships} ({connectedEdges.length})</p>
                    ...
                    return (
                      <div key={i} className="text-xs p-2 rounded bg-muted/50 space-y-1 hover:bg-muted transition-colors">
                        ...
                        <div className="flex gap-2 pt-0.5">
                          <Badge variant="outline" className="text-[10px]">{t.novel.intimacy} {edge.intimacy}%</Badge>
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground">{t.novel.clickNodeForDetails}</p>
                <p className="text-xs text-muted-foreground/60 mt-1">{t.novel.dragToAdjust}</p>
              </div>
            )}
          </CardContent>
        </Card>
...
function DetailPanel({ detail, connectedEdges, graphData, careers: _careers, roleLabels, edgeCategoryMeta, t }: {
  detail: CharacterDetail; connectedEdges: GraphEdge[]; graphData: GraphData | null; careers: CareerListResponse;
  roleLabels: Record<string, string>; edgeCategoryMeta: Record<EdgeCategory, { label: string; color: string; order: number }>;
  t: any;
}) {
    ...
    <div className="space-y-3 pr-2">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">{isOrg ? t.novel.organizations : roleLabels[detail.role_type || ''] || t.novel.characters}</p>
        </div>
      </div>
      ...
      {!isOrg && (
        <div className="flex flex-wrap gap-1.5">
          ...
          {detail.age && <Badge variant="outline" className="text-[10px]">{detail.age}{t.novel.yearsOld}</Badge>}
        </div>
      )}
      ...
      {!isOrg && (
        <>
          {detail.personality && <InfoField label={t.novel.personalityTraits} value={detail.personality} rows={2} />}
          {detail.appearance && <InfoField label={t.novel.appearanceTraits} value={detail.appearance} rows={2} />}
          {detail.motivation && <InfoField label={t.novel.actionMotivation} value={detail.motivation} rows={2} />}
          {detail.backstory && <InfoField label={t.novel.backgroundStory} value={detail.backstory} rows={3} />}
        </>
      )}

      {isOrg && (
        <>
          {detail.motivation && <InfoField label={t.novel.orgPurpose} value={detail.motivation} rows={2} />}
          {detail.backstory && <InfoField label={t.novel.orgBackground} value={detail.backstory} rows={3} />}
        </>
      )}
      ...
      <div>
        ...
        {connectedEdges.length === 0 ? (
          <p className="text-xs text-muted-foreground">{t.novel.noRelatedRelationships}</p>
        ) : (
...
function OrganizationNode(_data: { data: { label: string; roleType: string; isOrganization: boolean } }) {
  return (
    <div className="px-3 py-2 rounded-lg border shadow-sm cursor-pointer transition-all hover:shadow-md"
      ...
      <span className="font-semibold text-sm leading-tight text-center max-w-full truncate">{_data.data.label}</span>
    </div>
  );
}
...
function CareerNode({ data }: { data: { label: string; careerType: 'main' | 'sub' } }) {
  ...
  return (
    <div className="px-2 py-1 rounded-md border shadow-sm"
      ...
      <span className="text-[9px]" style={{ color, opacity: 0.7 }}>{data.label}</span>
