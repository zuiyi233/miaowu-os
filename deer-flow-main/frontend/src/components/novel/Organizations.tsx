...
export function Organizations({ projectId }: OrganizationsProps) {
  ...
  const loadOrgs = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${backendBase}/api/organizations/project/${projectId}`,
        { credentials: "include" },
      );
      if (!res.ok) return;
      const data = await res.json();
      setOrgs(Array.isArray(data) ? data : []);
      if (Array.isArray(data) && data.length > 0 && !selectedOrgId) {
  ...
  const loadMembers = async (orgId: string) => {
    try {
      const res = await fetch(
        `${backendBase}/api/organizations/${orgId}/members`,
        { credentials: "include" },
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${backendBase}/api/organizations/${selectedOrgId}`,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="flex items-center gap-2 text-lg font-semibold">
          <Building2 className="h-5 w-5" /> 势力/组织管理
        </h2>
            ...
            <>
              ...
              <CardContent className="space-y-3">
                ...
                {selectedOrg.location && (
                  <Info label="所在地" value={selectedOrg.location} />
                )}
                {selectedOrg.motto && (
                  <Info label="格言" value={selectedOrg.motto} />
                )}
                <Separator />
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-xs">职位</TableHead>
                        <TableHead className="text-xs">等级</TableHead>
                        <TableHead className="text-xs">忠诚度</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {members.map((m) => (
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      power_level: Number(e.target.value),
                    }))
                  }
                />
              </div>
