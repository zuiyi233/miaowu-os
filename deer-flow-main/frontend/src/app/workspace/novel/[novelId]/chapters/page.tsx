...
export default function NovelChaptersPage() {
  ...
  if (!novelId) {
    return <div className="flex h-full items-center justify-center text-muted-foreground">{t.novel.loading}</div>;
  }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t.novel.chapterManagement}
          </CardTitle>
          <CardDescription>{t.novel.createChapterDescription}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-col gap-2 sm:flex-row">
            <Input
              ...
              placeholder={t.novel.newChapterTitlePlaceholder}
              onKeyDown={(event) => {
            ...
            <Button onClick={handleCreateChapter} disabled={addChapter.isPending || !newChapterTitle.trim()}>
              <Plus className="mr-1 h-4 w-4" />{t.novel.addChapter}
            </Button>
          </div>

          {isLoading ? <p className="text-sm text-muted-foreground">{t.novel.loadingChapters}</p> : null}

          {!isLoading && chapters.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t.novel.noChaptersYet}</p>
          ) : null}
              ...
              <div key={chapter.id} className="flex items-center justify-between rounded-md border p-3">
                <div className="min-w-0 space-y-1">
                  <p className="truncate text-sm font-medium">
                    {t.novel.chapterOrder.replace('{n}', String(index + 1))} · {chapter.title}
                  </p>
                ...
                <div className="ml-2 flex items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (window.confirm(t.novel.confirmDeleteChapter)) {
                        deleteChapter.mutate({ novelId, chapterId: chapter.id });
                      }
                    }}
                  >
